...
static int v4l2_reqbufs_mmap(struct v4l2_device *dev, int nbufs)
{
    ...
    /* Map the buffers. */
    dev->mem = (struct buffer *)calloc(req.count, sizeof dev->mem[0]);
    if (!dev->mem) {
...
static int v4l2_open(struct v4l2_device **v4l2, char *devname, struct v4l2_format *s_fmt)
{
    ...
    dev = (struct v4l2_device *)calloc(1, sizeof *dev);
    if (dev == NULL) {
        ret = -ENOMEM;
        goto err;
    }
...
static int uvc_open(struct uvc_device **uvc, char *devname)
{
    ...
    dev = (struct uvc_device *)calloc(1, sizeof *dev);
    if (dev == NULL) {
        ret = -ENOMEM;
        goto err;
    }
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "bdf:hi:m:n:o:r:s:t:u:v:")) != -1) {
        ...
        case 'o':
            ...
            uvc_io_method = (enum io_method)atoi(optarg);
        ...
        case 's':
            ...
            speed = (enum usb_device_speed)atoi(optarg);
            break;
